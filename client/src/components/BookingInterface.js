...
const BookingInterface = () => {
  ...
  const fetchLiveSlotStatus = async () => {
    try {
      ...
      console.log('DEV fetchLiveSlotStatus start:', selectedDateStr);
      const response = await bookingAPI.getSlotStatus(selectedDateStr);
      const { availableSlots, maxSlots } = response.data;
      console.log("DEV -->",response.data,availableSlots,maxSlots);
  ...
  useEffect(() => {
    if (selectedDate) {
      console.log('DEV useEffect(fetchLiveSlotStatus) triggered');
      fetchLiveSlotStatus();
